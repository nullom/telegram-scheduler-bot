...
class TelegramSchedulerBot:
    ...
    def setup_logging(self) -> None:
        ...
        root_logger.setLevel(
            self.config.get_log_level_numeric()
            if self.config
            else logging.INFO
        )
    ...
    async def initialize_components(self) -> None:
        ...
        try:  # Load configuration
            ...
            self.logger.info(
                (
                    f"Database initialized: {total_messages} total messages, "
                    f"{unsent_messages} unsent"
                )
            )
            ...
            self.logger.info(
                f"Bot connected: @{bot_info.username} (ID: {bot_info.id})"
            )

            # Initialize handlers
            self.handlers = TelegramHandlers(
                self.database, self.config.owner_id
            )
            ...
            self.logger.info(
                f"Added {len(self.handlers.get_handlers())} message handlers"
            )
    ...
    async def start_bot(self) -> None:
        ...
        try:
            ...
            self.logger.info(
                f"Scheduler started with {job_info['total_jobs']} jobs"
            )
            self.logger.info(
                f"Next message send: {job_info['next_message_send']}"
            )
            ...
            self.logger.info(
                "Bot started successfully and is polling for updates"
            )
            # Send startup notification to owner
            try:
                next_send = (
                    job_info["next_message_send"].strftime("%H:%M")
                    if job_info["next_message_send"]
                    else "No messages"
                )
                startup_message = (
                    "🤖 Telegram Scheduler Bot Started!\n\n"
                    f"📊 Status:\n"
                    f"• Database: {self.database.get_message_stats()[1]} "
                    "unsent messages\n"
                    f"• Scheduler: {job_info['sending_jobs']} sending jobs "
                    "active\n"
                    f"• Next send: {next_send}\n\n"
                    "Ready to receive and schedule your messages!"
                )

                await self.bot.send_message(
                    chat_id=self.config.owner_id, text=startup_message
                )
                self.logger.info("Startup notification sent to owner")

            except TelegramError as e:
                self.logger.warning(
                    f"Could not send startup notification: {e}"
                )
        ...
        try:
            ...
            if self.bot and self.config:
                try:
                    ...
                    await self.bot.send_message(
                        chat_id=self.config.owner_id, text=shutdown_message
                    )
                    self.logger.info("Shutdown notification sent to owner")

                except TelegramError as e:
                    self.logger.warning(
                        f"Could not send shutdown notification: {e}"
                    )
