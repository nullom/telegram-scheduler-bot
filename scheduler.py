...
class MessageScheduler:
    ...
    async def _send_scheduled_message(self) -> None:
        ...
        try:
            ...
            if success:
                ...
                self.logger.info(
                    (
                        f"Successfully sent and removed {message_type} "
                        f"message (ID: {message_id})"
                    )
                )
            else:
                self.logger.error(
                    f"Failed to send {message_type} message (ID: {message_id})"
                )
    ...
    async def _send_text_message(self, message: dict) -> bool:
        ...
        try:
            await self.bot.send_message(
                chat_id=self.channel_id, text=message["content"]
            )
            return True
    ...
    async def _send_photo_message(self, message: dict) -> bool:
        ...
        try:
            await self.bot.send_photo(
                chat_id=self.channel_id, photo=message["file_id"]
            )
            return True
    ...
    async def _send_voice_message(self, message: dict) -> bool:
        ...
        try:
            await self.bot.send_voice(
                chat_id=self.channel_id, voice=message["file_id"]
            )
            return True
    ...
    async def _send_audio_message(self, message: dict) -> bool:
        ...
        try:
            await self.bot.send_audio(
                chat_id=self.channel_id, audio=message["file_id"]
            )
            return True
    ...
    async def _send_document_message(self, message: dict) -> bool:
        ...
        try:
            await self.bot.send_document(
                chat_id=self.channel_id, document=message["file_id"]
            )
            return True
    ...
    async def _cleanup_old_messages(self) -> None:
        ...
        try:
            deleted_count = self.database.cleanup_old_messages(
                self.cleanup_days
            )

            if deleted_count > 0:
                self.logger.info(
                    f"Cleanup completed: removed {deleted_count} old messages"
                )
            else:
                self.logger.info(
                    "Cleanup completed: no old messages to remove"
                )
    ...
    async def send_immediate_test(self) -> bool:
        ...
        try:
            ...
            await self.bot.send_message(
                chat_id=self.channel_id, text=test_message
            )
