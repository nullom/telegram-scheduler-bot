...
class DatabaseManager:
    ...
    def store_text_message(self, content: str) -> bool:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                conn.commit()
                self.logger.info(f"Text message stored successfully (ID: {cursor.lastrowid})")
                return True
    ...
    def store_url_message(self, url: str, original_text: str = None) -> bool:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                cursor = conn.cursor()
                cursor.execute(
                    "INSERT INTO messages (message_type, content, url) VALUES (?, ?, ?)",
                    ("url", original_text, url)
                )
                conn.commit()
                self.logger.info(f"URL message stored successfully (ID: {cursor.lastrowid})")
                return True
    ...
    def store_photo_message(self, file_id: str) -> bool:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                cursor = conn.cursor()
                cursor.execute(
                    "INSERT INTO messages (message_type, file_id) VALUES (?, ?)",
                    ("photo", file_id)
                )
                conn.commit()
                self.logger.info(f"Photo message stored successfully (ID: {cursor.lastrowid})")
                return True
    ...
    def store_voice_message(self, file_id: str) -> bool:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                conn.commit()
                self.logger.info(f"Voice message stored successfully (ID: {cursor.lastrowid})")
                return True
    ...
    def store_audio_message(self, file_id: str) -> bool:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                conn.commit()
                self.logger.info(f"Audio message stored successfully (ID: {cursor.lastrowid})")
                return True
    ...
    def store_document_message(self, file_id: str, file_name: str = None) -> bool:
        """
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                cursor = conn.cursor()
                cursor.execute(
                    "INSERT INTO messages (message_type, file_id, content) VALUES (?, ?, ?)",
                    ("document", file_id, file_name)
                )
                conn.commit()
                self.logger.info(f"Document message stored successfully (ID: {cursor.lastrowid})")
                return True
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                if cursor.rowcount > 0:
                    ...
                    self.logger.warning(f"Message {message_id} not found for marking as sent")
                    return False
                    
        except sqlite3.Error as e:
            self.logger.error(f"Failed to mark message {message_id} as sent: {e}")
            return False
    ...
    def delete_sent_message(self, message_id: int) -> bool:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                cursor = conn.cursor()
                cursor.execute(
                    "DELETE FROM messages WHERE id = ? AND sent_at IS NOT NULL",
                    (message_id,)
                )
                
                if cursor.rowcount > 0:
                    conn.commit()
                    self.logger.info(f"Message {message_id} deleted from database")
                    return True
                else:
                    self.logger.warning(f"Message {message_id} not found or not sent yet")
                    return False
    ...
    def get_message_stats(self) -> Tuple[int, int]:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                # Get unsent count
                cursor.execute("SELECT COUNT(*) FROM messages WHERE sent_at IS NULL")
