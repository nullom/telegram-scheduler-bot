...
class TelegramHandlers:
    ...
    def _log_message_received(self, update: Update, message_type: str) -> None:
        ...
        self.logger.info(
            (
                f"Received {message_type} message from owner "
    ...
    async def start_command(
        ...
        if not update or not update.effective_user or not update.message:
            self.logger.warning(
                "Received update without user or message information"
            )
            return
    ...
    async def status_command(
        ...
        if not self._is_owner(user_id):
            self.logger.warning(
                f"Unauthorized status command from user {user_id}"
            )
            await update.message.reply_text(
                "Access denied. This bot is private."
            )
    ...
    async def help_command(
        ...
        if not self._is_owner(user_id):
            self.logger.warning(
                f"Unauthorized help command from user {user_id}"
            )
            await update.message.reply_text(
                "Access denied. This bot is private."
            )
        ...
        help_message = (
            ...
            "• Audio files\n"
            "• Documents\n\n"
            "The bot will automatically store your messages and send them to "
            "the channel at scheduled times."
        )
    ...
    async def handle_text_message(
        ...
        if not self._is_owner(user_id):
            self.logger.warning(
                f"Unauthorized text message from user {user_id}"
            )
            return
        ...
        if self.database.store_text_message(text):
            self.logger.info(
                f"Text message stored successfully: {text[:50]}..."
            )
            if urls:
    ...
    async def handle_photo_message(
        ...
        if not self._is_owner(user_id):
            self.logger.warning(
                f"Unauthorized photo message from user {user_id}"
            )
            return
    ...
    async def handle_voice_message(
        ...
        if not self._is_owner(user_id):
            self.logger.warning(
                f"Unauthorized voice message from user {user_id}"
            )
            return
    ...
    async def handle_audio_message(
        ...
        if not self._is_owner(user_id):
            self.logger.warning(
                f"Unauthorized audio message from user {user_id}"
            )
            return
    ...
    async def handle_document_message(
        ...
        if not self._is_owner(user_id):
            self.logger.warning(
                f"Unauthorized document message from user {user_id}"
            )
            return

        if not update.message.document:
            self.logger.warning(
                "Received document handler call without document"
            )
            return
        ...
        if self.database.store_document_message(file_id, file_name):
            ...
            await update.message.reply_text(
                f"📄 Document '{file_name}' saved successfully!"
            )
        else:
            self.logger.error(
                f"Failed to store document message: {file_name} ({file_id})"
            )
            await update.message.reply_text(
    ...
    async def handle_other_messages(
        ...
        self.logger.info(
            f"Unsupported message type received from owner (ID: {user_id})"
        )
        await update.message.reply_text(
    ...
    async def stats_command(
        ...
        if not self._is_owner(user_id):
            self.logger.warning(
                f"Unauthorized stats command from user {user_id}"
            )
            await update.message.reply_text(
                "Access denied. This bot is private."
            )
